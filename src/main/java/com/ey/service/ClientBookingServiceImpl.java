...
public class ClientBookingServiceImpl implements ClientBookingService {
	...
	public ResponseEntity<?> createBooking(BookingCreateRequest request, String email) {
		...
		logger.info("Create booking request received by client email=" + email + ", eventId=" + request.getEventId()
		...
		try {
			...
			Event event = eventRepository.findByIdAndClient_Id(request.getEventId(), client.getId()).orElseThrow(() -> {
				logger.warn("Event not found or not owned. eventId=" + request.getEventId() + ", clientId="
						+ client.getId());
			...
			if (bookingRepository.existsByEvent_IdAndVendor_Id(event.getId(), vendor.getId())) {
				logger.warn("Duplicate booking detected eventId=" + event.getId() + ", vendorId=" + vendor.getId());
			...
			logger.info("Booking created successfully id=" + saved.getId() + ", eventId=" + saved.getEvent().getId()
					+ ", vendorId=" + saved.getVendor().getId() + ", status=" + saved.getStatus());

			BookingResponse resp = BookingMapper.toResponse(saved);
			return ResponseEntity.status(HttpStatus.CREATED).body(resp);

		} catch (ClientUnauthorizedException | EventNotFoundException | VendorNotFoundException
