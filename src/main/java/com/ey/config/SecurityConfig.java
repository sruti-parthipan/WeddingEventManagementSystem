	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http, AuthenticationManager authManager)
		...
		http.csrf(csrf -> csrf.disable())
				...
				.authorizeHttpRequests(auth -> auth
						...
						// ---------------- PUBLIC ----------------
						.requestMatchers("/api/auth/**").permitAll().requestMatchers("/api/admin/register").permitAll()
						.requestMatchers("/api/client/register").permitAll().requestMatchers("/api/vendor/register")
						.permitAll().requestMatchers("/api/forgot-password").permitAll()
						.requestMatchers("/api/reset-token").permitAll()

						// ---------------- CLIENT REVIEW POST  ----------------
						.requestMatchers(HttpMethod.POST, "/api/client/reviews/**").hasRole("CLIENT")

						// ---------------- CLIENT GET views (Admin & Client can view vendor/event
						// lists) ----------------
						...
						.requestMatchers(HttpMethod.PUT, "/api/vendor/bookings/*/accept",
								"/api/vendor/bookings/accept/*", "/api/vendor/bookings/*/cancel",
				...
				.addFilter(jwtAuthFilter).addFilterBefore(new JwtAuthorizationFilter(jwtUtil, multiUserDetailsService),
